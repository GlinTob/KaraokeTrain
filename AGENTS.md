...
- Cadena vigente (sep-2026): `index.html` → `style.css?v=6`, `cloudflare-storage.js?v=3`, `supabase-config.js?v=3`, `worklets.js?v=5`, `script.js?v=13`, `pitch-shifter-processor.js?v=10`. `script.js` → `config.js?v=7` (8×), `biblioteca.js?v=4`, `afinador.js?v=1`, `cambiar-tono.js?v=6`, `karaoke.js?v=10`. `karaoke.js` → `config.js?v=7`, `afinador.js?v=1`, `biblioteca.js?v=4`, `worklets.js?v=5`. `biblioteca.js` → `karaoke.js?v=10`. `cambiar-tono.js` → `karaoke.js?v=10`, `biblioteca.js?v=4`, `worklets.js?v=5`. `estudio.js` → `afinador.js?v=1`. Worker de audio: `audio-controller.js` → `audio-processor-worker.js?v=2` (clásico, sin importers con `?v=`). `worklets.js` carga `vocal-gate-processor.js?v=2` y `pitch-shifter-processor.js?v=10` (vía `index.html`).
...
## Módulos clave
- `index.html` define `window.__PITCH_WORKLET_URL__`; `modules/worklets.js` carga los worklets (`registerProcessor`): pitch-shifter y **vocal-gate**. El procesador vocal (vocal-processor.js / vocal-settings.js / liveAudioService.js) se ELIMINÓ (sep-2026): no reintroducir; el mix karaoke balancea voz 55% / pista 45% con gains y **limpia la voz automáticamente** en el render offline (`karaoke.js`): cadena voz = BiquadFilter highpass 90Hz → `vocal-gate-processor.js` (noise gate/expansor por RMS: abre > −28 dB, cierra < −44 dB, piso −42 dB, attack 30ms/release 600ms, ganancia suavizada por muestra) → DynamicsCompressor (−18, knee 10, ratio 4) → gain 0.55. Todo interno, sin controles de usuario.
