...
export async function showTab(tabId) {
  ...
  try {
    if (normalizedTabId === "config") {
      ...
      const { loadTrackOptionsInKaraoke, loadKaraokeSong } = await import("./modules/karaoke.js?v=11");
...
export async function drawKaraokeMonitor(currentTime, currentFreq, currentFreq2 = 0) {
  ...
  if (!_karaokeRenderPincel) {
    const { drawKaraokeMonitor: renderPincel } = await import('./modules/karaoke.js?v=11');
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  safeAdd("karaokeTrackSelect", "change", async () => {
    const { loadKaraokeSong } = await import("./modules/karaoke.js?v=11");
