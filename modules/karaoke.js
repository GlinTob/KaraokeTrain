...
const track = $("karaokeTrack") \vert{}\vert{} $("trackPlayer");
...
if (typeof newConfig.duoSplitMode === "boolean") {
karaokeDuoSplitMode = newConfig.duoSplitMode;
}
...
$${ word: seg.text, start: seg.start, end: seg.end, note: seg.note }$$
...
words.forEach((w) => {
  ...
  if (y !== null) {
    ...
    if (typeof ctx.roundRect === "function") {
      ...
      ctx.rect(startX, y - 6, barWidth, 12);
    }
    ctx.fill();
...
$$DuoSplit$$
 ...
 No hay Mic 2 seleccionado; el rastro P2 no se podrá dibujar.");
return;
}
...
$$DuoSplit$$
 ...
 Pitch tracking del Mic 2 iniciado");
} catch (e) {
...
export function drawTeleprompter(ctx, segments, currentTime, x, y, width, height) {
...
ctx.fillText(prefijo + (currentSeg.text || ""), x + width / 2, y + height / 2 - 15);

if (nextSeg) {
ctx.font = "italic 18px Arial";
ctx.fillStyle = "#888888";
ctx.fillText(nextSeg.text || "", x + width / 2, y + height / 2 + 20);
}
...
const track = $("karaokeTrack") || $("karaokeAudio") || $("audioKaraoke") \vert{}\vert{} $("trackPlayer");
const currentTime = track ? track.currentTime : 0;
...
const audioFile = $("ultrastarAudioFile")?.files?.
...
const vocalsFile = $("ultrastarVocalsFile")?.files?.
...
if (typeof cargarLetrasEnMonitor === "function") {
  cargarLetrasEnMonitor(karaokeLoadedLyrics);
}
...
export function loadTrackOptionsInKaraoke() {
console.log("🎵 Pistas de karaoke listas.");
}
...
if (resultDiv) {
resultDiv.innerHTML = "Uniendo la pista y tu voz. Esto puede tardar unos segundos...";
}
...
if (resultDiv) {
resultDiv.innerHTML = "❌ Hubo un error al mezclar los audios.";
}
} finally {
if (btn) {
