...
export function selectAvatar(character) {
  ...
  if (infoEl) {
    infoEl.innerHTML = `
      ...
      <div class="avatar-selected-sub">${character.name}</div>
...
export function toggleMic2Visibility() {
  ...
  const mic2Group = document.getElementById("mic2Group");
...
export async function testMicrophone(micNumber) {
  ...
  const select = document.getElementById(selectId);
  const levelBar = document.getElementById(levelId);
...
// Placeholder for showSaveNotification function if it exists elsewhere
...
export function selectAvatar(character) {
  ...
  if (typeof window.updateMonitorConfig === 'function') {
    window.updateMonitorConfig(character);
  }
}
